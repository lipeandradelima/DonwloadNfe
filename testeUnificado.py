...
Versão Final e Otimizada v4:
- CORREÇÃO DE BUG: Refatorada a função de leitura de Excel com 'openpyxl'
  para ser compatível com o modo de apenas leitura (read_only), corrigindo
  o erro 'has no attribute iter_cols'.
- Exclusão Manual de Chaves: Coluna "Ação" com um ícone 'X' para remover chaves.
- Botão de Ação Dinâmico: Alterna entre "Iniciar" e "Parar" Download.
- Teste de Senha Integrado: Botão "Testar" para validar o certificado.
...
SAIDA_DIR = os.path.join(BASE_DIR, "XMLs_Baixados")
...
def baixar_xml_por_chave(chave: str, cnpj: str, cert_files: Tuple[str, str], config: Dict) -> Dict[str, any]:
    ...
    try:
        ...
        if cStat == '138':
            ...
            if docZip_element is not None:
                ...
                return {"status": "Sucesso", "motivo": f"Salvo como: {nome_arquivo}"}
            else:
...
class App(tk.Tk):
    ...
    def selecionar_pfx(self):
        """Abre uma janela para o usuário selecionar o arquivo de certificado .pfx."""
        filepath = filedialog.askopenfilename(title="Selecione o Certificado Digital",
                                              filetypes=[("Certificados PFX", "*.pfx")])
        if filepath:
            self.entry_pfx.delete(0, tk.END);
            self.entry_pfx.insert(0, filepath)
            self._validar_e_preencher_cnpj()

    def _validar_e_preencher_cnpj(self):
        """Valida o certificado/senha e preenche o CNPJ automaticamente."""
        ...
        if info and info.get("cnpj"):
            ...
            self.entry_cnpj.insert(0, info["cnpj"])
        elif info:
    ...
    def set_controls_state(self, state: str):
        ...
        for widget in [self.btn_carregar_chaves, self.btn_selecionar_pfx, self.entry_pfx, self.entry_senha,
                       self.entry_cnpj, self.btn_testar_senha]:
    ...
    def _preparar_execucao(self) -> Optional[Tuple[str, Tuple[str, str]]]:
        """Valida os inputs e prepara os arquivos temporários do certificado."""
        pfx_path, password = self.entry_pfx.get(), self.entry_senha.get()
        ...
        try:
            with tempfile.NamedTemporaryFile(delete=False, suffix='.key', mode='wb') as key_file:
                self.key_temp_path = key_file.name
                key_file.write(cert_info["key_pem"])
            with tempfile.NamedTemporaryFile(delete=False, suffix='.cer', mode='wb') as cert_file:
                self.cert_temp_path = cert_file.name
                cert_file.write(cert_info["cert_pem"])
        except Exception as e:
            messagebox.showerror("Erro", f"Falha ao criar arquivos de certificado temporários: {e}")
            return None

        return cnpj, (self.cert_temp_path, self.key_temp_path)

    def _finalizar_execucao(self):
        """Limpa os arquivos temporários e reseta a interface."""
    ...
    def carregar_chaves_do_arquivo(self):
        """Lê um arquivo .xlsx usando openpyxl e carrega as chaves na lista."""
        ...
        try:
            ...
            # Procura nas primeiras 20 linhas para identificar a coluna correta
            ...
            if coluna_de_chaves_idx == -1:
                return messagebox.showerror("Erro",
                                            "Nenhuma coluna com chaves de 44 dígitos foi encontrada nas primeiras 20 linhas.")

            # Itera em todas as linhas para extrair as chaves da coluna identificada
            for row in sheet.iter_rows(min_row=1, values_only=True):
                ...
                if isinstance(valor_celula, str):
                    ...
                    if len(chave_limpa) == 44:
                        chaves_encontradas.append(chave_limpa)

            self.tree_chaves.delete(*self.tree_chaves.get_children())
            self.chaves_a_processar.clear()

            for chave in chaves_encontradas:
                if os.path.exists(os.path.join(SAIDA_DIR, f"{chave}-nfe.xml")):
                    self.tree_chaves.insert("", "end", values=("Já Baixado", chave, "-", ""), tags=('info',))
                else:
    ...
    def _ao_clicar_na_arvore(self, event):
        """Gerencia cliques na tabela para a ação de exclusão."""
        ...
        if col_heading == "Ação":
            ...
            if item_para_remover:
                ...
                if self.chaves_a_processar:
                    ...
                    self.status_label.config(text="Nenhuma chave para baixar.")
                    self.btn_acao_download.config(state='disabled')
    ...
    def get_config(self) -> Dict:
        """Retorna um dicionário com as configurações atuais da aplicação."""
        return {
            'UF_CODIGO_IBGE': self.UF_CODIGO_IBGE,
            'TP_AMB': self.TP_AMB,
            'URL_WEBSERVICE': self.URL_WEBSERVICE,
            'TIMEOUT': self.TIMEOUT
        }

    def parar_download(self):
        """Sinaliza para a thread de download parar."""
    ...
    def _processo_de_download(self, cnpj: str, cert_files: Tuple[str, str], config: Dict):
        ...
        try:
            ...
            for i, item in enumerate(self.chaves_a_processar):
                ...
                resultado = baixar_xml_por_chave(item['chave'], cnpj, cert_files, config)

                status = resultado.get("status", "Erro")
                motivo = resultado.get("motivo", "Sem detalhes")
                tag = ('sucesso',) if status == "Sucesso" else ('erro',)
    ...
    def monitorar_fila(self):
        """Verifica a fila de atualizações da thread e as aplica na GUI."""
        try:
        ...
        if self.download_em_andamento:
            self.after(100, self.monitorar_fila)
